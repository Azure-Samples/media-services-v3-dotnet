...
const string CustomTransform = "Custom_CopyCodecWithProxy";
    ...
    var transform = await mediaServicesAccount.GetMediaTransforms().CreateOrUpdateAsync(
        ...
        new MediaTransformData
        {
            Outputs =
            {
                ...
                // This also generates a fast proxy. See notes in the readme.md file on constraints and use case.
                new MediaTransformOutput(
                    preset: new BuiltInStandardEncoderPreset("saasProxyCopyCodec")
                    )
                {
            ...
            Description = "A custom transform with copy codec and a proxy"
        });
