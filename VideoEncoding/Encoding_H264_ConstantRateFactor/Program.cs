...
const string CustomTransform = "Custom_H264_CRF";
...
static async Task<MediaTransformResource> CreateTransformAsync(MediaServicesAccountResource mediaServicesAccount, string transformName)
{
    ...
    var transform = await mediaServicesAccount.GetMediaTransforms().CreateOrUpdateAsync(
        ...
        new MediaTransformData
        {
            Outputs =
            {
                // Create a new TransformOutput with a custom Standard Encoder Preset using the H.264 (H264Layer) codec with Constant Rate Factor
                    ...
                    preset: new StandardEncoderPreset(
                        codecs: new MediaCodecBase[]
                        {
                            ...
                            new H264Video
                            {
                                ...
                                // We use CRF : Constant Rate Control. It's similar to constrained VBR
                                // Other modes are ABR (Average Bitrate, which is the default) and CBR (Contant Bitrate)
                                RateControlMode = H264RateControlMode.Crf,
                        
                                // Add H264Layer. Assign a label that you can use for the output filename.
                                Layers =
                                {
                                    new H264Layer(bitrate: 2000000)
                                    {
                                        Width = "1280",
                                        Height = "720",
                                        Label = "720p-CRF", // This label is used to modify the file name in the output formats
                                        
                                        // The range of CRF value is between 0 and 51, where lower values would result in better quality, at the expense of higher file sizes.
                                        // Higher values mean more compression, but at some point quality degradation will be noticed. Default value is 23.
                        ...
                        formats: new MediaFormatBase[]
                        {
                            ...
                            new Mp4Format(filenamePattern: "Video-{Basename}-{Label}-{Bitrate}kbps{Extension}"),
            ...
            Description = "A custom encoding transform with 1 MP4 CRF bitrate"
        });
