...
const string CustomTransform = "Custom_OverlayImage_2";
...
string inputVideoAssetName = $"input-{uniqueness}";
string inputOverlayAssetName = $"logo-{uniqueness}";
...
var inputVideoAsset = await CreateInputAssetAsync(mediaServicesAccount, inputVideoAssetName, InputMP4FileName);

// Create a new input Asset and upload the specified overlay graphic image file into it.
var inputOverlayAsset = await CreateInputAssetAsync(mediaServicesAccount, inputOverlayAssetName, OverlayFileName);
...
var jobInput = new MediaJobInputs
{
    Inputs =
    {
        new MediaJobInputAsset(assetName: inputVideoAsset.Data.Name),
        new MediaJobInputAsset(assetName: inputOverlayAsset.Data.Name)
        {
            Label = OverlayLabel
        }
    }
...
if (job.Data.State == MediaJobState.Error)
{
    ...
    await CleanUpAsync(transform, job, inputVideoAsset, outputAsset, null, stopStreamingEndpoint, null);
    return;
}
...
await CleanUpAsync(transform, job, inputVideoAsset, outputAsset, streamingLocator, stopStreamingEndpoint, streamingEndpoint);
...
static async Task<MediaTransformResource> CreateTransformAsync(MediaServicesAccountResource mediaServicesAccount, string transformName)
{
    ...
    var transform = await mediaServicesAccount.GetMediaTransforms().CreateOrUpdateAsync(
        ...
        new MediaTransformData
        {
                        ...
                        codecs: new MediaCodecBase[]
                        {
                            ...
                            new H264Video
                            {
                                ...
                                Layers =
                                {
                                    ...
                                    new H264Layer(bitrate: 600000) // Adding a second layer to see that the image also is scaled and positioned the same way on this layer. 
                                    {
                                        Profile = H264VideoProfile.Baseline,
                                        Width = "480",
                                        Height = "270",
            ...
            Description = "A simple custom encoding transform with overlay"
        });
