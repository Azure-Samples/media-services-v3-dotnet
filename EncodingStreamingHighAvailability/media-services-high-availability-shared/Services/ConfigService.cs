    ...
    public class ConfigService : IConfigService
    {
        ...
        public async Task LoadConfigurationAsync()
        {
            ...
            using (var keyVaultClient = new KeyVaultClient(new KeyVaultClient.AuthenticationCallback(azureServiceTokenProvider.KeyVaultTokenCallback)))
            {
                ...
                var secret = await keyVaultClient.GetSecretAsync($"https://{this.keyVaultName}.vault.azure.net/secrets/MediaServiceInstanceConfiguration").ConfigureAwait(false);
                this.MediaServiceInstanceConfiguration = JsonConvert.DeserializeObject<Dictionary<string, MediaServiceConfigurationModel>>(secret.Value);
